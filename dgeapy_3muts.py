...
try:
    ...
    # type of error.
    df_recovered = dgeapy.recover_corrupted_file(filename=DATAFRAME_FILE_NAME)
...
dgeapy.add_fold_change_columns(dataframe=df)
# Add "Regulation" column with values.
dgeapy.add_regulation_columns(dataframe=df)
...
# generated df.
sub_dataframes = dgeapy.generate_sub_dataframes(
        dataframe=df,
...
dgeapy.add_fold_change_columns(df_to_merge)
dgeapy.add_regulation_columns(df_to_merge)
sub_dataframes[df_to_merge_mutant] = df_to_merge

dfs_directory = dgeapy.mk_new_dir("dataframes")
volcano_directory = dgeapy.mk_new_dir("volcano_plots")
...
for mutant in MUTANT_SAMPLES:
    ...
    column_names_to_check = dgeapy.column_names_to_check(
    ...
    # filtered dataframe.
    sub_dataframes_filtered[mutant] = dgeapy.filter_FC_PVALUE_PADJ(
            dataframe=sub_dataframes[mutant],
...
venn_directory = dgeapy.mk_new_dir(
...
up_regulation_labels = dgeapy.get_gene_ids_list_for_intersections(
...
down_regulation_labels = dgeapy.get_gene_ids_list_for_intersections(
